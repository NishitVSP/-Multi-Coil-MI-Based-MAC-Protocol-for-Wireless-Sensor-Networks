...
## Overview
Implementation of a Multi-Coil Magneto-Inductive (MI) MAC protocol for wireless sensor networks using NS-3 network simulator.

## Paper Reference
Based on: "Multi-Coil MI Based MAC Protocol for Wireless Sensor Networks"
- Niaz Ahmed, Yahong Rosa Zheng, David Pommerenke
- Missouri University of Science & Technology

## Key Features
- **Three-Coil System**: X, Y, Z orthogonal coils for omni-directional communication
- **Five-State Machine**: Idle, Receive, Channel Sensing, Data Acquire, Transmit
- **RSSI-Based Coil Selection**: Automatically selects strongest coil pair
- **Energy Efficient**: Optimized state transitions minimize energy consumption
- **Spatial Reuse**: Different node pairs can communicate on different coil orientations

## Protocol Packets
1. **REV (Reservation)**: 13-byte handshake packet sent 3 times (once per coil)
2. **ACK (Acknowledgment)**: 5-byte response with selected coil pair
3. **DATA**: Variable length (1-16 bytes) sensor data packet

## Files
- `scratch/mi-mac-demo.cc` - Basic demonstration of the MAC protocol
- `scratch/mi-mac-comparison.cc` - Comparison with traditional CSMA/CA MAC
- `results/` - Simulation output logs

## Requirements
- NS-3 version 3.46
- Ubuntu/Linux environment (WSL compatible)
...
```bash
cd ~
wget https://www.nsnam.org/releases/ns-3.46.tar.bz2
...
Basic demonstration:
```bash
./ns3 run scratch/mi-mac-demo
```


Comparison with CSMA/CA:
```bash
./ns3 run scratch/mi-mac-comparison
```


Save output to file:
```bash
./ns3 run scratch/mi-mac-comparison > results/output.log
```
...
## YOU CAN READ THE ORIGINAL DOCUMENTATION (README.md FILE) OF NS-3 BELOW. 
